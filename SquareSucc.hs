...
{-
symmetry :: Int -> (IArray.Array Int [[Int]]) -> ([Block], Bottoms) -> [[Block]]
symmetry rectHeight sumDict (blocks, bottoms) =
  finishFill rectHeight (blocks, bottoms) $ continueFill
  where 
...
searchRect' rectHeight sumDict (blocks, bottoms) =
  ...
  where
  --  bottomGroups = List.groupBy (on (&&) ((rectHeight/=) . getTop)) $ Zipper.toList bottoms 
  --  map (\btms->symmetry rectHeight sumDict (blocks,btms)) bottomGroups
    lowBottoms = Zipper.toMinZipperOn id $ Zipper.toList bottoms
    Bottom _ len = Zipper.cursor lowBottoms
    mayPrems = filter (null.(List.intersect blocks)) ((IArray.!) sumDict len)
    mayPrems' = concat $ map List.permutations mayPrems
